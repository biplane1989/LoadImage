...
object SDCardUtils {
    ...
    fun mkdirs(context: Context?, dir: File): Boolean {
        ...
        if (!res) {
            if (DocumentsUtils.isOnExtSdCard(dir, context)) {
                val documentFile = DocumentsUtils.getDocumentFile(dir, true, context)
    ...
    fun delete(context: Context?, file: File): Boolean {
        ...
        if (!ret && DocumentsUtils.isOnExtSdCard(file, context)) {
            val f = DocumentsUtils.getDocumentFile(file, false, context)
            if (f != null) {
                ret = f.delete()
            }
        }
    ...
    fun getInputStream(context: Context, destFile: File): InputStream? {
        ...
        try {
            if (!canWrite(destFile) && isOnExtSdCard(destFile, context)) {
                val file = DocumentsUtils.getDocumentFile(destFile, false, context)
    ...
    fun canWrite(context: Context?, file: File): Boolean {
        ...
        if (!res && DocumentsUtils.isOnExtSdCard(file, context)) {
            val documentFile = DocumentsUtils.getDocumentFile(file, true, context)
            res = documentFile != null && documentFile.canWrite()
        }
        return res
    }
    ...
    fun checkWritableRootPath(context: Context?, rootPath: String?): Boolean {
        ...
        return if (!root.canWrite()) {
            if (DocumentsUtils.isOnExtSdCard(root, context)) {
                val documentFile = DocumentsUtils.getDocumentFile(root, true, context)
